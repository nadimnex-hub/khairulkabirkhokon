      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 mb-6 rounded-full bg-primary/10 border border-primary/20">
            ...
            <span className="font-bengali text-sm text-primary font-medium">সর্বশেষ খবর</span>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {displayNews.map((item) => (
              <article key={item.id} className="group card-elevated overflow-hidden">
                {/* Image */}
                <div className="aspect-video bg-gradient-to-br from-primary/20 to-primary/5 rounded-xl mb-4 overflow-hidden">
                  {item.image_url ? (
                    <img 
                      src={item.image_url} 
                      alt={item.title}
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <span className="font-bengali text-4xl text-primary/30">📰</span>
                    </div>
                  )}
                </div>
                ...
                {/* Title */}
                <h3 className="font-bengali font-semibold text-lg text-foreground mb-2 group-hover:text-primary transition-colors">
                  {item.title}
                </h3>

                {/* Description */}
                <p className="font-bengali text-muted-foreground line-clamp-2 mb-4">
                  {item.description}
                </p>
